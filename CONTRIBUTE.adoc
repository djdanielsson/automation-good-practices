...
[source,yaml]
----
\include::examples/contribute/do/a_mini_playbook_example/playbook.yml[]
----
+
Even more examples...
====
...
== YAML examples in guidelines

Ansible YAML shown in the guidelines is stored in separate files and transcluded with `include::path/to/file.yml[]`.
This keeps examples lintable with `ansible-lint` and avoids duplicating content between docs and CI.

. Good (lintable) Ansible examples live in `.yml` files under `examples/`, or next to the guideline (for example `playbooks/` or `roles/`).
  Transclude them from AsciiDoc with `include::relative/path.yml[]`.
. Intentionally invalid or anti-pattern examples belong under a `bad/` directory segment (for example `examples/foo/bad/bar.yml`).
  `ansible-lint` excludes `**/bad/**` via `.ansible-lint`.
. Generic YAML that is not Ansible content (GitHub Actions workflows, release manifests, branch protection settings, and similar) may stay inline in AsciiDoc.
  Mark those blocks with `[source,yaml,lint=skip]` so pre-commit does not require a separate file.
. When showing AsciiDoc source inside a listing block (as in the template above), escape transclusion with a leading backslash: `\include::path[]`.
  Otherwise Asciidoctor expands the include while rendering the template.
